...
"----------------------------------------------------------------------
function! utils#tools#buffer_switch(bid)
...
" get content
"----------------------------------------------------------------------
function! utils#tools#list_buffer(switch)
	...
	for bid in bids
		...
		let cmd = 'call utils#tools#buffer_switch(' . bid . ')'
		if a:switch != ''
	...
	if exists('g:quickui_tools_width')
		let opts.w = quickui#utils#tools#tools_width()
	endif
	" let opts.syntax = 'cpp'
...
"----------------------------------------------------------------------
function! utils#tools#list_register()
endfunc
...
"----------------------------------------------------------------------
function! utils#tools#python_help(word)
...
" display messages
"----------------------------------------------------------------------
function! utils#tools#display_messages()
	let x = ''
	redir => x
	silent! messages
	redir END
	...
	if len(content) == 0
		call quickui#utils#tools#errmsg('Empty messages')
		return -1
	endif
...
" preview quickfix
"----------------------------------------------------------------------
function! utils#tools#preview_quickfix(...)
	if quickui#preview#visible()
		call quickui#preview#close()
		return 0
	endif
	if &bt != 'quickfix'
		call quickui#utils#tools#errmsg('Not in quickfix window !')
		return -1
	endif
	...
	if index < 1 || index > len(obj.items)
		call quickui#utils#tools#errmsg('No information in this line')
		return -2
	endif
...
" preview tag
"----------------------------------------------------------------------
function! utils#tools#preview_tag(tagname)
	let tagname = (a:tagname == '')? expand('<cword>') : a:tagname
	if tagname == ''
		call quickui#utils#tools#errmsg('Error: empty tagname')
		return 0
	endif
	...
	if len(ptag.taglist) == 0
		call quickui#utils#tools#errmsg('E257: preview: tag not find "' . tagname . '"')
		return 1
	endif
	...
	if !filereadable(filename)
		call quickui#utils#tools#errmsg('E484: Can not open file '.filename)
		return 2
	endif
	if !has_key(taginfo, 'line')
		call quickui#utils#tools#errmsg('Error: no "line" information in your tags, regenerate with -n')
		return 3
	endif
	...
	if display != 0
		call quickui#utils#tools#print(text, 1)
	endif
	return 0
endfunc
...
"----------------------------------------------------------------------
function! utils#tools#display_help(tag)
	...
	if !exists('s:help_tags')
		call quickui#utils#tools#errmsg('Sorry, not find help tags in $VIMRUNTIME')
		return -1
	endif
	if !has_key(s:help_tags, a:tag)
		call quickui#utils#tools#errmsg('E149: Sorry, no help for '. a:tag)
		return -2
	endif
	...
	if !filereadable(name)
		call quickui#utils#tools#errmsg('E484: Sorry, cannot open file '.name)
		return -3
	endif
	let content = readfile(name)
	let opts = {'syntax':'help', 'color':'QuickPreview', 'close':'button'}
	let opts.title = 'Help: ' . fnamemodify(name, ':t')
	let g:utils#tools#hint = item[1]
	let opts.command = ['silent! exec g:utils#tools#hint']
...
function! utils#tools#clever_context(name, content, opts)
...
function! utils#tools#clever_listbox(name, content, opts)
	let opts = deepcopy(a:opts)
	let opts.index = get(s:previous_cursor, a:name, -1)
	let opts.keep_name = a:name
...
" terminal
"----------------------------------------------------------------------
function! utils#tools#terminal(name)
	if !exists('g:quickui_terminal_tools')
		let g:quickui_terminal_tools = {}
	endif
	if !has_key(g:quickui_terminal_tools, a:name)
		call quickui#utils#tools#errmsg('ERROR: tool ' . a:name . ' not defined !')
		return -1
	endif
	let tools = g:quickui_terminal_tools[a:name]
	if !has_key(tools, 'cmd')
		call quickui#utils#tools#errmsg('ERROR: key cmd not present in tool ' . a:name)
		return -1
	endif
	let opts = {}
	let cmd = tools.cmd
